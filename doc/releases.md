...
20 years after its creation Param has reached version 2.0.0! Can you guess when Param 3.0 will be released?

This release includes some major enhancements and features, including a new game-changer reactive API. It also includes various enhancements, bug fixes, improvements to the documentation and infrastructure. Being a major release it includes some breaking changes, although we have tried to limit them to a minimum.  We would like to thank @minimav for their first contribution, and @droumis, @Hoxbro, @jbednar, @maximlt, @philippjfr and @sdrobert for their contributions.

### Major enhancements and features

- Several functionalities developed on top of Param appeared in the HoloViz ecosystem in the last years. Panel added the `pn.bind` utility to bind a function to references like Parameters, methods/functions decorated with `param/pn.depends` and widgets. Bound functions when processed by the Panel machinery are re-executed automatically when one of their references is updated and their display is refreshed with the new returned value. hvPlot added `.interactive` API, allowing to build pipelines that transform an input datastructure of a type supported by the library (e.g. Pandas DataFrame). Similarly to `pn.bind`, `.interactive` accepts references being injected in the pipeline and its output is re-evaluted on their updates to refresh some visualization. These functionalities proved very useful over the years, we have therefore decided to move them down the stack directly in Param (on which they are built upon!) so they can be better integrated in the HoloViz ecosystem. Param holds now the implementation of `param.bind` and `param.rx` (`rx` for Reactive eXpression), `param.rx` being an evolved and improved implementation of hvPlot `.interactive`'s API. To unify reactive expressions with the other ways of expressing interactivity in Param, Parameters and bound functions have gained the `.rx` method to turn them into reactive expressions. We are very much impressed by the capabilities of reactive expressions and it is not impossible that we end up moving them to their own package in the hope of making them more easily discoverable! ([#460](https://github.com/holoviz/param/pull/460), [#842](https://github.com/holoviz/param/pull/842), [#841](https://github.com/holoviz/param/pull/841), [#843](https://github.com/holoviz/param/pull/843), [#844](https://github.com/holoviz/param/pull/844), [#845](https://github.com/holoviz/param/pull/845), [#846](https://github.com/holoviz/param/pull/846), [#849](https://github.com/holoviz/param/pull/849), [#847](https://github.com/holoviz/param/pull/847), [#850](https://github.com/holoviz/param/pull/850), [#851](https://github.com/holoviz/param/pull/851))
- Parameterized classes and instances have received an HTML repr that is displayed automatically in a notebook on `.param` access, it shows a table of all the Parameters of the class/isntance, their state, type range and docstring on hover. It is likely we will improve the content and design of this repr, please give us your feedback! ([#425](https://github.com/holoviz/param/pull/425), [#781](https://github.com/holoviz/param/pull/781), [#821](https://github.com/holoviz/param/pull/821), [#831](https://github.com/holoviz/param/pull/831))
- In a hierarchy of Parameterized classes, Parameter slot values are now all inherited correctly. Let's say we have class `B` being a subclass of `A` itself being a subclass of `param.Parameterized`. If `A` defines `x = Number(1, bounds=(0, 10))` and `B` defines `x = Number(2)`, `B.param['x'].bounds` is going to be inherited from `A` and equal to `(0, 10)`. This mechanism used to work only partially, it was leveraging `None` as a sentinel except that `None` can sometimes be a valid value for a Parameter slot. Instead now the Parameter signatures all default to the `Undefined` sentinel which allows for `None` to be inherited too. You should not have to interact directly with `Undefined`, unless if you are creating Parameter classes ([#605](https://github.com/holoviz/param/pull/605), [#771](https://github.com/holoviz/param/pull/771), [#791](https://github.com/holoviz/param/pull/791))

### Enhancements

- Shallow-copy mutable containers in slot values, so when a class defines a Parameter that has one slot set with a mutable container value (e.g. `Selector(objects=a_list)`), that container isn't shared between the class and its subclasses and instances ([#826](https://github.com/holoviz/param/pull/826))
- Add `ListProxy` wrapper around `Selector.objects` (with forward and backward compatibility) to easily update `objects` and watch `objects` updates ([#598](https://github.com/holoviz/param/pull/598), [#825](https://github.com/holoviz/param/pull/825))
- In the spirit of cleaning up the Parameterized namespace debuted in version 1.7.0, the remaining private attributes haven been collected under two private namespaces `_param__private` and `_param__parameters` ([#766](https://github.com/holoviz/param/pull/766), [#790](https://github.com/holoviz/param/pull/790))
- Allow `.param.update` to be used as a context manager for temporary updates ([#779](https://github.com/holoviz/param/pull/779))
- Overriding the `name` Parameter is taken into account at the class and instance level ([#740](https://github.com/holoviz/param/pull/740))
- Improve Parameter signatures for static and dynamic code analysis ([#742](https://github.com/holoviz/param/pull/742))
...
- Only generate the Parameter docstring in an IPython context ([#774](https://github.com/holoviz/param/pull/774))
...
- `Integer`: accept `numpy.integer` values ([#735](https://github.com/holoviz/param/pull/735))
- `Range`: stricter validation of the slot values ([#725](https://github.com/holoviz/param/pull/725), [#824](https://github.com/holoviz/param/pull/824))
- `Path`: add `check_exists` attribute and raise an error if `path` is not found on parameter instantiation ([#800](https://github.com/holoviz/param/pull/800))
...
- Mitigating performance regressions in 2.0 ([#839](https://github.com/holoviz/param/pull/839))
...
- Don't automatically set `instantiate` to True when `constant` is True ([#776](https://github.com/holoviz/param/pull/776))
...
- Drop support of Python 2.7, 3.6 and 3.8 and upgrade the code base ([#741](https://github.com/holoviz/param/pull/741), [#784](https://github.com/holoviz/param/pull/784))

### Breaking changes

- While it's a major improvement to the definition of Parameters, properly inheriting Parameter slots can result in some Parameter slot values being different in Param 2 compared to Param 1 that was sometimes silently not inheriting slot values. On top of that, Param 2.0 adds a validation step of the default value of a Parameter after the inheritance mechanism has completed if its type has changed (e.g. `x` in class `A` is a `Number` and in class `B(A)` is an `Integer`) or one of its slot values has changed ([#812](https://github.com/holoviz/param/pull/812), [#820](https://github.com/holoviz/param/pull/820))
- `Parameterized` methods that were deprecated since Param 1.7.0 have finally been removed, they are mostly available on the `.param` namespace ([#592](https://github.com/holoviz/param/pull/592))
- No longer allow to set class attributes during initialization and don't warn when setting non-Parameter class attributes ([#729](https://github.com/holoviz/param/pull/729))
...
- Some removals were deemed harmless and executed without deprecation period:
  - Removed `bounds` slot from `Boolean` and `Event` ([#744](https://github.com/holoviz/param/pull/744), [#755](https://github.com/holoviz/param/pull/755))
  - Removed Parameter `_internal_name` slot ([#796](https://github.com/holoviz/param/pull/796))
  - Removed `Parameterized.pprint`, `Parameterized._pprint`, `Parameterized.script_repr`, `ParameterizedFunction.script_repr`, `Time.next`, and moved `Parameterized.state_pop` and `Parameterized.state_push` to the `.param` namespace ([#767](https://github.com/holoviz/param/pull/767))

### Deprecations

We continue to clean up Param's API ([#734](https://github.com/holoviz/param/pull/734), [#751](https://github.com/holoviz/param/pull/751), [#768](https://github.com/holoviz/param/pull/768), [#797](https://github.com/holoviz/param/pull/797), [#834](https://github.com/holoviz/param/pull/834), [#838](https://github.com/holoviz/param/pull/838)) but have decided to do it in a gentle way, emitting deprecation warnings for a period of time before proceeding with removals. You will find below the complete list of deprecation warnings added in Param 2.0. Note that while these warnings are meant to stay in place for a sufficient period of time (to be determined), it is **very likely** that some if not all removals will happen **before Param 3.0** is released. Don't loose too much time before taking care of these warnings when you encounter them!
...
- Parameter signature:
  ...
  - For `Selector` parameters that accept `objects` as first positional argument, and `ClassSelector` parameters that accept `class_` as first positional argument, passing any argument by position is deprecated:
    ...
    - It's possible that in the future the signature of these two parameters will be aligned with the other parameters to accept `default` as first and only positional argument.
- Parameter slots:
