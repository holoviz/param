...
class TestDateRange(unittest.TestCase):
    ...
    def test_validate_bounds_wrong_type_lower(self):
        msg = re.escape("DateRange parameter lower bound must be a date/datetime, not <class 'str'>.")
        with pytest.raises(ValueError, match=msg):
            param.DateRange(bounds=('a', dt.datetime(2017,2,27)))

    def test_validate_bounds_wrong_type_upper(self):
        msg = re.escape("DateRange parameter upper bound must be a date/datetime, not <class 'str'>.")
        with pytest.raises(ValueError, match=msg):
            param.DateRange(bounds=(dt.datetime(2017,2,27), 'b'))

    def test_validate_softbounds_wrong_type_lower(self):
        msg = re.escape("DateRange parameter lower softbound must be a date/datetime, not <class 'str'>.")
        with pytest.raises(ValueError, match=msg):
            param.DateRange(softbounds=('a', dt.datetime(2017,2,27)))

    def test_validate_softbounds_wrong_type_upper(self):
        msg = re.escape("DateRange parameter upper softbound must be a date/datetime, not <class 'str'>.")
        with pytest.raises(ValueError, match=msg):
